...
void OLED_print_char(char c) {
	...
	for (int row = 0; row < 8; row++) {
		...
		XMEM_write(pgm_read_byte(&font8[c][row]), 0x200);
	}
}
...
void OLED_refresh(void){
	for (int page = 0; page < 8; page++){
		XMEM_write(0xB0 + page, 0xB0 + page);
		for (int column = 0; column < 128; column++){
			uint8_t lowerCol = column & 0x0F;
			uint8_t upperCol = column >> 4;
			
			XMEM_write(lowerCol, lowerCol);		// Set lower column start address
			XMEM_write(0x10 + upperCol, 0x10 + upperCol);
...
void OLED_clearAll(void) {
	for (uint8_t page = 0; page < 8; page++) {
		XMEM_write(0xB0 + page, 0xB0 + page); // Set page in command register
		XMEM_write(0x10, 0x10); // Set upper column start address
		XMEM_write(0, 0); // Set lower column start address
		for (uint8_t currentCol = 0; currentCol < 128; currentCol++) {
			XMEM_write(0x0,0x200);
		}
	}
}
